#Function to automate the installation of multiple beats agents as Windows services. Currently supports the Windows ZIP (x86_64) platform.
#For more info : https://www.elastic.co/downloads/beats/
...
[CmdletBinding()]
    param (
        ...
        [Parameter(
            Position=2,
            Mandatory=$true,
            HelpMessage="Provide the installation version with the following format (8.2.2).")]
        [ValidatePattern('[0-9]')]
        [ValidateNotNullOrEmpty()] 
    ...
    $Folders = @("$env:ProgramData\$($Beat)","$env:ProgramData\$($Beat)\logs")
        ...
        try {
            ...
            Write-Warning -Message "Unable to set $($Beat) service to delayed start."
            break
        }
    }
}
